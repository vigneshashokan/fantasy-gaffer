...
6. `src/app/(onboarding)/connect-team.tsx` — the button that submits the entered team ID (`onPress={onContinue}`, guarded by `disabled={validating}`): `testID="connect-team-submit"`; the confirming-stage button (`onPress={onLink}`, "Yes, link team"): `testID="connect-team-confirm"`. Read the file to place these on the actual pressable elements (they may be `PillBtn`s — which now accept `testID`).
