...
- **Buildable now (no external blockers):** #73 verify biometric on dev build (**its #44 blocker is resolved** — dev-client builds exist and are already in use by the Maestro suite, so this is actionable now, just needs the manual on-device walkthrough) · #71 universal-links code (needs AASA/assetlinks hosting; domains registered — see the paid-account risk flagged above) · #91 `[Phase 2]` pitch/player visual bugs (needs on-device screenshots per the issue itself) · #77 `[Last Leg]` data protection & regional compliance strategy (real remaining scope — DPA/SCC disclosure text, export flow, `privacy@` routing — needs the operator's legal/business judgment calls, not something to decide unilaterally).
