...
- **#48 Maestro E2E — SHIPPED (PR #151, squash `365cabb`).** See the **E2E (Maestro)** section above for the suite shape + gotchas. Bonus outcome: first live run caught + fixed the prod connect-team confirm 404 (teamPreview object-in-URL, broken since PR #84). CI follow-up = #152 (designed, not built).

- **Buildable now (no external blockers):** #49 first-run onboarding · #73 verify biometric on dev build · #50 i18n (optional; would also localise the #46 legal copy) · #71 universal links (needs AASA/assetlinks hosting; domains registered).
