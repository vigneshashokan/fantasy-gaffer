...
- **Buildable now (no external blockers):** #49 first-run onboarding · #73 verify biometric on dev build (**its #44 blocker is resolved** — dev-client builds exist and are already in use by the Maestro suite, so this is actionable now, just needs the manual on-device walkthrough) · #71 universal links (needs AASA/assetlinks hosting; domains registered).
