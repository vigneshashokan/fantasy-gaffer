...
- **Confirmed 2026-07-08: neither the Apple Developer Program ($99/yr) nor Google Play Console ($25) is enrolled yet** — the single biggest fan-out blocker left in Phase 5. Gates #44, #45, #14 `[Last Leg]` Apple Sign-In (also an App Store *policy* requirement since the app offers Google sign-in), likely #71 (iOS Associated Domains needs a paid account, not just a free Apple ID — unverified, flagged as a risk), and the iOS half of #158's on-device push validation.
- **#73 biometric — RE-SCOPED, code done, on-device pass left open `[Blocked — paid Apple account]`.** The original ask ("verify biometric on dev build") turned into a redesign: verification proved the signed-out session-restore model (spec `docs/superpowers/specs/2026-06-09-biometric-unlock-design.md`) **unreachable** — GoTrue revokes any signed-out session server-side, including `scope:'local'` (stored access token → `403`, refresh token → `400`). Re-scoped to a **lock on an already-live session**, resolved once at cold start (`AppGate`/`resolveLock` in `src/app/_layout.tsx`, `src/store/biometricStore.ts`, `src/components/auth/LockScreen.tsx`) — see `docs/auth-biometric.md` and spec `docs/superpowers/specs/2026-07-25-biometric-app-lock-design.md`. Shipped + unit-tested (`src/__tests__/appGate.test.tsx`, `src/__tests__/components/auth/lockScreen.test.tsx`, `src/__tests__/biometricStore.test.ts`); no E2E coverage is possible (Maestro can't satisfy the system Face ID sheet, see `docs/e2e.md`). **Left open:** the remaining scope is a manual on-device pass of the Face ID success/cancel/lockout paths (acceptance-criteria table in the spec), which needs a physical dev build — gated on the same Apple Developer Program enrollment as #44/#45, not a code dependency.
- **Buildable now (no external blockers):** #71 universal-links code (needs AASA/assetlinks hosting; domains registered — see the paid-account risk flagged above) · #91 `[Phase 2]` pitch/player visual bugs (needs on-device screenshots per the issue itself) · #77 `[Last Leg]` data protection & regional compliance strategy (real remaining scope — DPA/SCC disclosure text, export flow, `privacy@` routing — needs the operator's legal/business judgment calls, not something to decide unilaterally).
- **Gated on paid store accounts:** #44 EAS production build (profiles exist; the `.ipa`/`.aab` ACs need Apple + Play provisioning) · #45 store listings.
